...
json.dump({"_meta": meta, "tables": cat}, open(f"{root}/docs/catalog/bq-catalog.json", "w"), ensure_ascii=False, indent=0)
allow = {k: [c["name"] for c in v["columns"]] for k, v in cat.items()}
json.dump({"generated_date": gen, "tables": allow}, open(f"{root}/docs/catalog/bq-catalog.allowlist.json", "w"), ensure_ascii=False)
