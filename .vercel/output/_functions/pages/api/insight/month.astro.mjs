...
async function runBQ(bigquery, stage, query, params, rid) {
  const t0 = Date.now();
  try {
    ...
    e.stage = stage;
    e.bq = { code: err?.code, errors: err?.errors };
    throw e;
  }
}
const GET = async ({ request, locals }) => {
  ...
  const hasWindowStart = urlCheck.searchParams.has("window_start_date") || urlCheck.searchParams.has("window_start_date");
  if (!hasWindowStart) {
  ...
  let bq_total_ms = 0;
  try {
    let asYmd = function(v) {
      return bqDateToYmd(v);
    }, weatherSeverityFromWmo = function(code) {
      const c = Number(code);
      if (!Number.isFinite(c)) return -1;
      if (c >= 95 && c <= 99) return 5;
      if (c >= 80 && c <= 82) return 4;
      if (c >= 71 && c <= 77) return 3;
      if (c >= 61 && c <= 67 || c >= 51 && c <= 57) return 2;
      if (c === 45 || c === 48) return 1;
      return 0;
    }, topNByScore = function(days2, n, dir) {
      const xs = days2.map((d) => ({ ...d, date_ymd: asYmd(d?.date) })).filter((d) => d.date_ymd && Number.isFinite(Number(d?.opportunity_score_final_local)));
      xs.sort((a, b) => {
      ...
      return xs.slice(0, n);
    }, worstNByWeather = function(days2, n) {
      const xs = days2.map((d) => ({
        ...d,
      ...
      return xs.slice(0, n);
    }, topNByCompetition = function(days2, n) {
      const xs = days2.map((d) => ({ ...d, date_ymd: asYmd(d?.date) })).filter((d) => d.date_ymd && Number.isFinite(Number(d?.events_within_10km_count)));
      xs.sort((a, b) => Number(b.events_within_10km_count) - Number(a.events_within_10km_count));
      return xs.slice(0, n);
    }, topNByRiskBucket = function(days2, n) {
      const xs = days2.map((d) => ({ ...d, date_ymd: asYmd(d?.date) })).filter((d) => d.date_ymd && String(d?.relative_rank_bucket ?? "") === "risk").filter((d) => Number.isFinite(Number(d?.opportunity_score_final_local)));
      xs.sort((a, b) => Number(a.opportunity_score_final_local) - Number(b.opportunity_score_final_local));
      return xs.slice(0, n);
    }, bqScalarToString = function(v) {
    ...
    }, dedupeDaysByDate = function(days2) {
      const byDate = /* @__PURE__ */ new Map();
      for (const d of Array.isArray(days2) ? days2 : []) {
        const k = bqDateToYmd(d?.date);
        if (!k) continue;
        if (!byDate.has(k)) byDate.set(k, d);
      }
      const out = Array.from(byDate.entries()).sort((a, b) => a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0).map(([, v]) => v);
      return out;
    }, extractSpecialDays = function(days2) {
      const out = [];
      for (const d of Array.isArray(days2) ? days2 : []) {
      ...
      return dict[v] ?? v;
    }, hasMeaningfulMeteoSignal = function(days2) {
      const xs = Array.isArray(days2) ? days2 : [];
      for (const d of xs) {
    ...
    requireString(locals.clerk_user_id, "locals.clerk_user_id");
    ...
    const window_start_date = requireString(
      url.searchParams.get("window_start_date") ?? url.searchParams.get("window_start_date"),
      "window_start_date"
    );
    ...
    async function runBQ_timed(stage, query, params) {
    ...
    const window = Array.isArray(windowRows) && windowRows.length > 0 ? windowRows[0] : null;
    const days = Array.isArray(daysRows) ? daysRows : [];
    const ctxList = Array.isArray(ctxRows) ? ctxRows : [];
    ...
    console.log("[MONTH][CTX RAW]", {
      ...
      region_code_insee: location_context?.region_code_insee ?? "__missing__",
      city_id: location_context?.city_id ?? "__missing__",
      city_name: location_context?.city_name ?? "__missing__"
    });
    const region_code_insee_raw = location_context?.region_code_insee ?? null;
    ...
    const regionRows = region_code_insee ? await runBQ_timed("fetch_region_ai_context_daily", sqlRegion, {
      region_code_insee,
      window_start_date
    }) : [];
    const regionByDate = /* @__PURE__ */ new Map();
    for (const r of Array.isArray(regionRows) ? regionRows : []) {
      const k = bqDateToYmd(r?.date);
      if (!k) continue;
      if (!regionByDate.has(k)) regionByDate.set(k, r);
    }
    const days_enriched = days.map((day) => {
      const k = bqDateToYmd(day?.date);
      const r = k ? regionByDate.get(k) : null;
      if (!r) return day;
      return {
        ...day,
        // Names come from region context
        public_holiday_name_fr: r.public_holiday_name_fr ?? null,
        school_holiday_name: r.school_holiday_name ?? null,
        // Commercial events
        is_commercial_event_flag_region: r.is_commercial_event_flag_region ?? false,
        commercial_event_names_region: Array.isArray(r.commercial_event_names_region) ? r.commercial_event_names_region : []
      };
    });
    ...
    for (const d of Array.isArray(days_deduped) ? days_deduped : []) {
      if (Array.isArray(d.commercial_event_names_region)) {
        d.commercial_event_names_region = d.commercial_event_names_region.map(translateEventLabelFr);
      }
    }
    ...
    console.log("[MONTH][REGION ENRICH]", {
      ...
      sample_enriched: days_enriched?.[0] ? {
        ...
        public_holiday_name_fr: days_enriched[0]?.public_holiday_name_fr ?? null,
        is_commercial_event_flag_region: days_enriched[0]?.is_commercial_event_flag_region ?? null,
        commercial_event_names_region_len: Array.isArray(days_enriched[0]?.commercial_event_names_region) ? days_enriched[0].commercial_event_names_region.length : null
      } : null
    });
    ...
    let shouldRunAI = computed_window.days_count === 30 && (computed_window.days_risk > 0 || hasMeaningfulMeteoSignal(days_deduped) || worst_competition_days.some((d) => Number(d?.events_within_10km_count ?? 0) > 0));
    ...
    if (!window) {
      ...
      try {
        const {
          ...
          key_takeaway: _drop_key_takeaway,
          // optional: keep if you want, I drop to avoid kitchen-like phrasing
          ...window_no_kitchen
        } = window ?? {};
        const month_ai_row = {
          ...window_no_kitchen,
        ...
        let ai_window_summary = null;
        let ai_special_days = null;
        ...
        console.log("[MONTH][AI SUBCALLS]", {
          ...
          ws_raw_text_len: typeof ai_window_summary?.raw_text === "string" ? ai_window_summary.raw_text.length : null,
          sd_ok: ai_special_days?.ok ?? null,
          sd_raw_text_len: typeof ai_special_days?.raw_text === "string" ? ai_special_days.raw_text.length : null
        });
      } catch (e) {
        ai = {
          ok: false,
    ...
    const ai_input = window && location_context ? {
      ...window ?? {},
      special_days,
      best_score_days,
      worst_score_days,
    ...
    return new Response(
      JSON.stringify({
        ...
        debug_month: include_debug ? {
          ...
          ai_output_keys: ai?.output && typeof ai.output === "object" ? Object.keys(ai.output) : null
        } : null,
        // ---- PROOF / TRACEABILITY ----
        trace: {
        ...
        ai_points_cles_text: (() => {
          ...
          function renderSpecialDaysRanges() {
            const items = Array.isArray(special_days) ? special_days : [];
            const rows = items.map((sd0) => {
              const date = typeof sd0?.date === "string" ? sd0.date.trim() : "";
              const labels = Array.isArray(sd0?.labels) ? sd0.labels.filter((x) => typeof x === "string" && x.trim()).map((x) => x.trim()) : [];
          ...
          function deterministicText() {
            ...
            if (Array.isArray(best_score_days) && best_score_days.length > 0) {
              const topDates = best_score_days.map((d) => bqDateToYmd(d?.date) ?? d?.date_ymd ?? null).filter((x) => typeof x === "string" && x.trim()).slice(0, 3);
          ...
          const candidates = [
            ...
            ...Array.isArray(ws?.key_facts) ? ws.key_facts.map((v, i) => ({ label: `ws.key_facts[${i}]`, value: v })) : [],
            { label: "sd.headline", value: sd?.headline },
            ...Array.isArray(sd?.special_days) ? sd.special_days.map((v, i) => ({ label: `sd.special_days[${i}]`, value: JSON.stringify(v) })) : []
          ];
