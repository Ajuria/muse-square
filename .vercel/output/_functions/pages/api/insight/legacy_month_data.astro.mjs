...
function normalizeTopDaysField(day, warnOnce) {
...
async function runBQ(bigquery, stage, query, params) {
  const t0 = Date.now();
  try {
...
const GET = async ({ request, locals }) => {
  try {
    let asYmd = function(v) {
      ...
      return dict[v] ?? v;
    }, computeOpportunityStats = function(days30) {
      const out = {
      ...
      return out;
    }, formatRangeYmd = function(d1, d2) {
      return d1 === d2 ? d1 : `${d1} → ${d2}`;
    }, buildPointsClesText = function(args) {
      const { days: days2, special_days: special_days2, window_start_date: window_start_date2, window_end_date } = args;
      const stats = computeOpportunityStats(days2);
      const daysCount = stats.days_count;
      const ab = stats.days_a + stats.days_b;
      const abShare = daysCount ? ab / daysCount : 0;
      const opportunities = abShare >= 0.7 ? "beaucoup d’opportunités" : abShare >= 0.45 ? "des opportunités variables" : "peu d’opportunités";
      const constraints = stats.days_risk > 0 ? "quelques contraintes d’organisation" : "peu de contraintes d’organisation";
      const lines = [];
      lines.push(`La période présente ${opportunities} et ${constraints}, avec les particularités suivantes:`);
      const weatherAlerts = (Array.isArray(days2) ? days2 : []).map((d) => {
        ...
        const mobLvl = Number(d?.mobility_alert_level);
        if (Number.isFinite(mobLvl) && mobLvl > 0) parts.push(`mobilité: alerte niveau ${mobLvl}`);
        const mobFlag = Boolean(d?.is_mobility_disruption_flag);
...
Alertes (météo et mobilité)`);
      ...
      lines.push(`
Jours spéciaux`);
      if (!Array.isArray(special_days2) || special_days2.length === 0) {
        lines.push(`- Pas de vacances, jours fériés ou événements commerciaux sur la période`);
      } else {
        for (const sd of special_days2.slice(0, 12)) {
          const d = typeof sd?.date === "string" ? sd.date : "";
          const labels = Array.isArray(sd?.labels) ? sd.labels.map((x) => typeof x === "string" ? x.trim() : "").filter(Boolean) : [];
          if (d && labels.length) lines.push(`- ${d}: ${labels.join(", ")}`);
        }
      }
      lines.push(`
Score d'opportunité sur la période`);
      lines.push(`- Fenêtre 30 jours: ${formatRangeYmd(window_start_date2, window_end_date ?? "")}`.trim());
    ...
    const window_start_date = resolveRows && resolveRows.length > 0 ? bqDateToYmd(resolveRows[0]?.window_start_date) : null;
    console.log("[MONTH] resolved window_start_date:", window_start_date, {
      raw: resolveRows?.[0]?.window_start_date
    });
    ...
    const days = (Array.isArray(daysRows) ? daysRows : []).map(
      (d) => normalizeTopDaysField(d, warnTopDays)
    );
    const location_context = Array.isArray(ctxRows) && ctxRows.length > 0 ? ctxRows[0] : null;
    const region_code_insee_raw = location_context?.region_code_insee ?? null;
    const region_code_insee = typeof region_code_insee_raw === "string" && region_code_insee_raw.trim() ? region_code_insee_raw.trim() : null;
    console.log("[MONTH][REGION] region_code_insee:", region_code_insee);
    const regionRows = region_code_insee ? await runBQ(bigquery, "fetch_region_ai_context_daily", sqlRegion, {
      region_code_insee,
      window_start_date
    }) : [];
    ...
    const points_cles_text = buildPointsClesText({
      ...
      special_days,
      window_start_date,
      window_end_date: bqDateToYmd(window?.window_end_date)
    });
    return new Response(
      JSON.stringify({
        ok: true,
