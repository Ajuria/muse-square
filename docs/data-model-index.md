...
| `fct_client_sales_by_category_daily` 🆕 **(HANDOFF — owner runs in dbt; model in `docs/dbt-handoff/`)** | table | location_id × transaction_date × item_category | raw.client_transactions | location_id, transaction_date, item_category, category_revenue, category_quantity, category_discount — preserves the product mix `fct_client_daily_performance` aggregates away; powers `insight/sales-breakdown.ts` ("what sold") |
