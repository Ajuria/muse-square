...
| `vw_insight_event_day_notes` 🆕 | view | note_id (une ligne par note, append-only — la dernière d'un jour fait foi) | stg_day_notes → analytics.day_notes | note_id, location_id, clerk_user_id, date, note_text, created_at | PR [ms_database#126](https://github.com/Ajuria/ms_database/pull/126) (07/09, à merger puis `dbt build --select +vw_insight_event_day_notes`) : la note-cause d'un jour écrite depuis l'état vide d'Explorer ; l'app écrit la table et la lit en direct (app-write, comme action_log) ; jointure prévue au résiduel du jour par (location_id, date) |
...
| `stg_day_notes` 🆕 | view | src:analytics.day_notes — PR [ms_database#126](https://github.com/Ajuria/ms_database/pull/126) (07/09, branche `explorer/day-notes` depuis `main`) ; la table existe en base |
