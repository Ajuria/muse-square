...
| `fct_client_commitment_outcomes` 🆕 | table | one row per resolved+done commitment | int_client_commitment_latest | commitment_id, location_id, user_id, source, authorship, action_type, driver, **origin_factor** (A↔B bridge), window_days, resolved_date, verdict, is_confounded, beat, effect_residual_pct, effect_residual_z |
...
| `fct_location_commitment_learning` 🆕 | table | location_id × **origin_factor** × action_type × window_days | fct_client_commitment_outcomes | location_id, origin_factor, action_type, window_days, source, authorship, done_count, beat_count, missed_count, confounded_count, avg/median_effect_residual_pct, last_resolved_date, has_sufficient_sample, is_proven_lift. Grain now factor-keyed (Engine-1 A↔B bridge); consumers roll up across factor for action_type-level advice |
