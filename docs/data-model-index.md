...
| `fct_client_hourly_signals_daily` 🆕 | table | location_id × transaction_date × transaction_hour — part de l'heure dans le CA du jour vs sa moyenne 30 j (z ≥ 2, ≥ 8 pts, n ≥ 20, calibré 23/08 : 16 % des jours-site) ; source `fct_client_hourly_sales` ; consommé par le CTE `hour_share_move` ; **journée arrêtée (06/09, HANDOFF-journee-arretee)** : ≥ 2 heures de queue à zéro après la dernière vente ⇒ `is_day_stopped`, `is_stop_row` (première heure à zéro), `last_sale_hour`, `typical_last_hour`, `stopped_hours`, `stop_hour`, `stopped_expected_revenue`/`_transactions` (attendus de la queue), `expected_revenue_until_stop`, `n_stopped_days_90d`, `stopped_dates_90d` — la queue n'est plus `is_hour_move` |
