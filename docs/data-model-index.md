...
| `mart.fct_client_family_price_daily` (cartes `family_price_move` et `family_discount_move`, blocs candidats 06/09 : un tir par site et par jour, famille au \|z\| le plus fort ; is_discount_move exige units ≥ 10 depuis le 06/09) | site × jour × famille | stg_client_transactions | prix réalisé (CA/unités), taux de remise ; base 28 j ; is_price_move (units ≥ 10), is_discount_move. **`has_fractional_units` (BOOL) : PAS encore en base — passation `docs/dbt-handoff/HANDOFF-prix-moyen-unite-2026-09-10.md` (10/09)** : une quantité non entière sur la famille, ce jour ou dans les 28 jours précédents ; `units` est alors une somme de poids et `realized_price` un prix au poids. La même passation l'ajoute au contrat de `vw_insight_event_family_price_daily` (`select *` sous contrat enforced : sans elle, son build échoue) |
