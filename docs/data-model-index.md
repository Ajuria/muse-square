...
>
> **Réponse aux signaux par famille — lot A dbt EN BASE (11/09, PR ms_database#146 `813176f`, run `70471897062675` ; état : `docs/reponse-aux-signaux-par-famille.md`)** : le moteur des classes de jours de l'app (`src/lib/kpi/dayClassRegistry.ts` → `analytics.day_class_impacts`) entre dans dbt — `int_location_day_cells` (site × jour de vente : cellule mois × semaine/week-end, appartenances, `n_memberships`), `int_location_day_class_membership` (site × jour × classe, 12 classes structurelles), `fct_location_day_class_impacts` (site × classe × base × métrique, port du store : parité 137/137 sur les classes déterministes, terciles EXACTS `percentile_disc`), `fct_client_family_day_class_response` (site × **famille** × classe × base × métrique : `family_units`, `family_basket`, `family_share`, `family_revenue`, `family_revenue_gap`, `family_avg_price`) et trois vues contrat `vw_insight_event_day_class_membership` / `…_day_class_impacts` / `…_family_day_class_response`. Seed `open_data.document_types` (source_system × document_code → kind, counts_as_sale) lu par `stg_client_transactions` : `is_invoiced = coalesce(règle de routage, counts_as_sale, true)` ; colonne `raw.client_transactions.document_type` EN BASE (ALTER 11/09). Vérifié 11/09 15 h 17 UTC : membership 2 804 lignes / 6 sites, impacts 224, famille 1 950 (f10c3e58 : 500, 9 familles, 6 métriques) ; staging 202 895 lignes = raw, 202 444 facturées inchangées. **Aucun lecteur app au 11/09** : le store app reste la source des pastilles et des coins. Grain pôle = ce mart regroupé par le mapping famille → pôle (lot B).

> **Composants des dispositifs permanents — chaîne semantic EN BASE** (appliquée par l'owner le 03/09 dans dbt Cloud IDE, PR #93 sur `main` ; passation `docs/dbt-handoff/HANDOFF-composants-2026-09-03.md`). `stg_client_commitments` et `int_client_commitment_latest` portent les 80 colonnes de la table (dont `components`, `operation_cost_eur`) ; `int_client_dispositif_components` (vue, grain dispositif × version × composant, `is_current_version`) ; `fct_client_dispositif_components` (fact mince en vue, dataset `mart`) ; `vw_insight_event_dispositif_components` (vue, 20 colonnes, contrat enforced : version courante + libellés et drapeau `provisoire` depuis les seeds) ; `vw_insight_event_commitment_memory` à 57 colonnes. Seeds `open_data.dispositif_types` (10) / `open_data.dispositif_roles` (19), copie générée de `src/lib/dispositifs/dispositifTypes.ts`. Vérifié en base le 03/09 (INFORMATION_SCHEMA, objets créés 11:09) et de bout en bout : deux versions sonde écrites par l'app lues dans les vues de production en 8,8 s (3 composants courants libellés, mémoire avec coût), sondes effacées. Tag `mart_dependent` : le job du matin recrée les vues ; les seeds se rechargent à la main (`dbt seed`) quand le registre app change. **Consommateurs app (03/09, § 5.5)** : `lib/poleReading.listPoleComponents` (→ résolveur d'entités, dashboard Piloter, onglet Pôles du compte, contexte de création), `lib/entityReading` (branche composant d'Explorer) lisent la vue semantic ; `api/commitments/evolution.ts` lit le JSON `components` de la version dans `vw_insight_event_commitment_memory`. Aucun lecteur de composants sur la table analytics ni sur le mart (cliquet `warehouseBoundary.guard`).

> **Lot dbt 2 — EN BASE (03/09, PR ms_database #95, `docs/dbt-handoff/HANDOFF-photos-signaux-2026-09-03.md`)** : `stg_dispositif_photos` → `int_client_dispositif_photos_latest` (photo courante par dispositif × version × composant, `items_effective` = confirmés sinon reconnus) → `fct_client_dispositif_photos` (fact mince, vue) → `vw_insight_event_dispositif_photos` (contrat 20 col. ; 24 depuis le 11/09 avec `exposition`, `levels`, `families_present`, `fixture_no`) ; `vw_insight_event_client_item_signals` = projection fidèle de `fct_client_item_signals_daily` (35 col., première surface semantic au grain article). Consommateur app : `lib/poleReading.buildPoleItemsReading` (document du pôle, Explorer). Vérifié en base 03/09 (objets créés 14:48 ; 8 973 lignes / 80 articles sur Muse Square).

> **`analytics.dispositif_photos`** (03/09, étape 4 incrément 1 — `docs/dispositifs-typologie-spec.md` § 5.2, owner D7) — app-write, APPEND-ONLY par `photo_id` (INSERT DML), lecture = dernière ligne par (`dispositif_id`, `version_no`, `component_key`), cluster `dispositif_id`. 21 colonnes : identité (photo, site, dispositif, version, composant, marche NULL), `gcs_uri` (bucket privé `ms-dispositif-photo`, EU — servi par l'API), `dispositif_type`/`dispositif_role` (registre), `status`, `checklist` (JSON clé → oui|non|non_visible, clés du registre seulement), `items_matched` (JSON codes de la liste du site), `items_confirmed`, `prices_seen`, `coverage_flag`, `model`, `prompt_version`, `created_by`, `created_at` — **+ 4 colonnes le 11/09 (lecture v2, owner ; 25 colonnes, `ALTER TABLE … ADD COLUMN IF NOT EXISTS`, sonde écrite/relue/effacée)** : `exposition` STRING (comptoir | vitrine | rayonnage | caisses_au_sol | ilot, registre `EXPOSITION_KINDS`, porte), `levels` INT64 (rayonnage seulement, null sinon), `families_present` ARRAY<STRING> (familles vendues du site, `listSiteFamilies` ; [] si aucune — jamais NULL), `fixture_no` INT64 (numéro sur le plan, donné par le client de l'API). **Une image où une personne est visible n'y entre jamais** (objet effacé, aucune ligne). Déclarée source dbt (`stg_dispositif_photos`, lot 2) — la chaîne `stg` → `int_client_dispositif_photos_latest` → `fct` → `vw_insight_event_dispositif_photos` porte les 4 colonnes v2 depuis le 11/09 (PR ms_database#144, fusion `60dc549` ; staging 25, intermédiaire et fact 27, vue 24 colonnes au contrat, mesuré après le run). **Aucun job ne reconstruit `stg_dispositif_photos`** (pas de fraîcheur déclarée sur la source, pas de tag `mart_dependent`) : toute colonne ajoutée à cette staging demande un run ponctuel `dbt run --select stg_dispositif_photos+` juste après la fusion (fait le 11/09, run dbt Cloud 70471897053059). Passation : `docs/dbt-handoff/HANDOFF-photos-v2-colonnes-2026-09-11.md`.

> **`analytics.action_commitments.measured_scope`** et **`raw.saved_items.measured_scope`** (07/09, `docs/dispositif-perimetre-mesure-spec.md` P0) — STRING JSON `{ kind familles|pole|articles, familles [{nom, nouvelle?}], pole_id, pole_nom, item_codes }`, ALTER IF NOT EXISTS vérifié en base, 22 lignes / 8 engagements migrés depuis `kpi_family` (`{"kind":"familles","familles":[{"nom":"Branded"}]}`). Exposé par `stg_client_commitments` → `vw_insight_event_commitment_memory` (PR [ms_database#128](https://github.com/Ajuria/ms_database/pull/128)).

> **`analytics.action_commitments.components`** (03/09, `docs/dispositifs-typologie-spec.md` § 3) — colonne 80, STRING (texte JSON), ALTER vérifié en base : la liste des COMPOSANTS d'un dispositif permanent, `[{key, type, role, label}]` — type/rôle = registre `src/lib/dispositifs/dispositifTypes.ts`, clé stable dans la chaîne de versions (la photo s'y rattachera), libellé libre. NULL sur une opération datée. Écrite par `POST /api/commitments` (branche pôle), lue par `GET /api/commitments/evolution`. **Pas encore dans le passthrough staging de `vw_insight_event_commitment_memory`** (dbt gelé, 27/08) — à ajouter au dégel.

> **`analytics.context_enrichment`** (étape 5 Explorer, 08/08) — app-write, store du crawl « contexte réel d'une journée » (`lib/ai/webContext.ts`, partagé dossier de carte + chat). Grain : APPEND-ONLY par `enrichment_id` (UUID), lecture = latest par `(location_id, date)` sous TTL 30 j — ne jamais agréger sans dédupliquer. Colonnes : `enrichment_id, location_id, date, takeaway, key_factors (JSON string), sources (JSON string d'URLs — le SEUL objet du produit à porter des URLs), enriched_at`. Champs produits par LLM + web_search : `<cite>` strippé à l'écriture ET à la lecture ; d'anciennes lignes portent des facteurs « [object Object] » (coercition ratée pré-08/08), filtrés à la lecture. Déclaration dbt `source()` : snippet prêt à coller dans l'IDE dbt Cloud (voir module-index, ligne webContext) quand une staging s'y adossera — aujourd'hui AUCUN modèle dbt ne le lit.
...
| `vw_insight_event_client_offering_daily` 🆕 | view | location_id × client_id × transaction_date × item_category, BORNÉ à aujourd'hui | fct_client_offering_daily | toutes les colonnes du mart + `invoice_count` (tickets contenant la famille, repli transaction_count), `ticket_revenue_avg` (panier du ticket ENTIER) | **EN BASE** 07/09 (PR [ms_database#127](https://github.com/Ajuria/ms_database/pull/127) mergée `8c474e1`, full-refresh du mart + build run 70471896423817) ; lue par dispositifFamille, offering, sales, weather, prompt (marges), dashboard (pôles), sales-report — plus aucune lecture du mart dans l'app |
| `vw_insight_event_client_hourly_daily` 🆕 | view | location_id × client_id × transaction_date × transaction_hour (jours DATÉS), BORNÉ à aujourd'hui | fct_client_hourly_sales | revenue, units, transactions, lines, visitors, avg_basket, conversion_rate | **EN BASE** 07/09 (même PR) ; lue par commitmentShape (heures d'une opération et de ses jours comparables) ; `vw_insight_event_client_hourly_profile` reste le PROFIL moyen **10/09 (ms_database#134) : `units` de fct_client_hourly_sales = Σ units_sold — une pesée compte pour une vente.** |
| `vw_insight_event_day_notes` 🆕 | view | note_id (une ligne par note, append-only — la dernière d'un jour fait foi) | stg_day_notes → analytics.day_notes | note_id, location_id, clerk_user_id, date, note_text, created_at | **EN BASE** 07/09 (PR [ms_database#126](https://github.com/Ajuria/ms_database/pull/126) mergée `cbde545`, build ponctuel run 70471896420048 vert, 6 colonnes typées vérifiées ; NB : ni `source_status:fresher+` ni `tag:mart_dependent` ne la reconstruisent — une vue ne change pas, un build ponctuel suffit) : la note-cause d'un jour écrite depuis l'état vide d'Explorer ; l'app écrit la table et la lit en direct (app-write, comme action_log) ; jointure prévue au résiduel du jour par (location_id, date) |
| `vw_insight_event_site_memory` 🆕 | view | (location_id, subject) — la DERNIÈRE ligne de chaque sujet d'un site quand elle n'est pas un retrait (superseded = false) | stg_explorer_site_memory → raw.explorer_site_memory | memory_id, location_id, subject, body, author_user_id, author_role, source, created_at | **PR OUVERTE** 11/09 ([ms_database#143](https://github.com/Ajuria/ms_database/pull/143), branche `feat/explorer-site-memory`, SQL rendus dry-run OK en EU) — vue NON construite tant que la PR n'est pas mergée et buildée ; l'app la lit par `src/lib/explorer/siteMemory.ts` (`lire_memoire`), rend `[]` en attendant. Contrat enforced, access public, test unique (location_id, subject). |
| `vw_insight_event_commitment_memory` 🆕 | view | commitment_id (état canonique d'une VERSION de dispositif — dédup tiebreak app, tous statuts, cancelled inclus) | stg_client_commitments → analytics.action_commitments | commitment_id, dispositif_id, version_no, parent_commitment_id, dispositif_plus, dispositif_why, dispositif_resources, adjustment_move, measured_metric, window_kind, window_start/end, threshold_*, kpi_*, verdict, verdict_basis + dispositif_nature/pole_families/attached_pole_id (pôles) + origin_kind/retro_note/creation_enjeu_* (55 col, contrat enforced ; staging = passthrough 78 col) |
...
| `vw_insight_event_competitor_offering_changes` 🆕 | view | competitor_id × item_norm | fct_competitor_offering_changes | competitor_id, competitor_name, item_norm, item, category, unit, currency, change_type, old_price_raw, old_price_numeric, old_price_qualifier, new_price_raw, new_price_numeric, new_price_qualifier, price_difference, price_pct_change, is_price_change, price_direction, previous_crawled_at, current_crawled_at, source_url, tarifs_url |
...
| `fct_client_day_residual` 🆕 | table, clustered | location_id × date | fct_client_sales_signals_daily, fct_location_context_features_daily | location_id, date, daily_revenue, expected_revenue, residual_pct, residual_z, is_revenue_down_residual, is_revenue_surge_residual, revenue_robust_z, is_revenue_down_anomaly, is_revenue_surge_anomaly. **Funnel (handoff 24/08, docs/dbt-handoff)** : expected_visitors/transactions/basket/conversion + \*_residual_z par facteur — LE « résultat habituel » de TOUS les facteurs (même méthode multi-facteurs qu'expected_revenue ; panier/conversion dérivés par identité, fermeture CA = passages × conversion × panier exacte par jour ; conversion = ratio 0-1 ; NULL si facteur absent, jamais inventé) |
| `fct_client_offering_daily` 🆕 (consommé aussi par `lib/dispositifFamille.ts` — I8 04/09 : `revenue_share` moyenné jours d'opération vs comparables ; porte depuis le 07/09 `invoice_count` et `ticket_revenue_avg` — PR ms_database#127, full-refresh fait ; l'app lit la vue semantic, plus le mart) | incr., insert_overwrite, part. daily | location_id × transaction_date × item_category | stg_client_transactions | location_id, client_id, transaction_date, item_category, line_count, units, revenue, avg_unit_price, promo_count, revenue_share, volume_share, revenue_rank **10/09 (#134) : `units` = Σ coalesce(units_sold, 1).** |
...
| `fct_competitor_directory` | table, clustered | competitor_id | int_competitor_directory, int_events_event_daily_enriched, src **raw_crawl**.competitor_events, src **raw_crawl**.watched_competitors | competitor_id, competitor_name, address, city, industry_code, industry_bucket, primary_audience, geo_point, google_place_id, google_rating, confidence_score, is_followed, first_crawled_at, last_crawled_at, commerce_type, commerce_gamme |
...
| `fct_competitor_threat_profile` | table, clustered | location_id × competitor_id | int_competitor_threat_profile | location_id, competitor_id, competitor_name, is_followed, audience_overlap_pct, industry_match_tier, seasonality_alignment, distance_km, threat_score, threat_level, location_commerce_type, location_commerce_gamme, competitor_commerce_type, competitor_commerce_gamme |
| `fct_competitor_offering_changes` 🆕 | table, clustered | competitor_id × item_norm | int_competitor_offering_changes, fct_competitor_directory | competitor_id, competitor_name, item_norm, item, category, unit, currency, change_type, old_price_raw, old_price_numeric, old_price_qualifier, new_price_raw, new_price_numeric, new_price_qualifier, price_difference, price_pct_change, is_price_change, price_direction, previous_crawled_at, current_crawled_at, source_url, tarifs_url |
...
| `fct_location_channel_monthly` 🆕 | table | location_id × channel_key × month_start | stg_client_transactions, src:analytics.party_directory, fct_location_channel_weekly, fct_location_sales_regime | location_id, channel_key, month_start, month_end, ca, active_days, invoices, top_parties, data_end, months_observed, iqr_ratio, sales_grain, is_weekly_judgeable, is_monthly_judgeable, baseline_median, baseline_months, month_ratio, month_state, is_run_start |
| `fct_location_channel_weekly` 🆕 | table | location_id × channel_key × week_start | stg_client_transactions, fct_location_sales_regime | location_id, channel_key, week_start, week_end, ca, active_days, data_end, weeks_observed, med_active_days, iqr_ratio, sales_grain, is_weekly_judgeable, baseline_median, baseline_weeks, week_ratio, week_state, is_run_start |
| `fct_location_client_patterns` 🆕 | table | location_id × party_code | stg_client_transactions, src:analytics.party_directory | location_id, party_code, party_label, source_location_id, data_end, orders_count, first_order, last_order, span_days, median_interval_days, silence_days, lateness_ratio, total_revenue, is_cadence_established, client_state |
| `fct_location_daily_action_candidates` 🆕 | table, part. date, clustered | date × location_id × action_type (dedup by suppression_key) | fct_location_context_features_daily, fct_location_events_radius_daily, fct_location_opportunity_score_daily, fct_location_weather_alerts_daily, fct_region_day_annotations_daily, dim_client_location, fct_competitor_events_conflicts, vw_insight_event_change_feed, fct_client_daily_performance, fct_competitor_threat_profile, fct_client_offering_profile, fct_competitor_directory, fct_location_impact_daily_calendar, fct_foreign_tourism_context_daily, fct_location_sales_regime, fct_location_client_patterns, fct_location_channel_weekly, fct_location_channel_monthly, fct_client_tickets_daily (06/09 : `tickets_lines_move`), fct_client_item_absence_daily (06/09 : `item_absent_regular`), fct_client_family_price_daily (06/09 : `family_price_move`, `family_discount_move`) ; 07/09 : `foreign_tourism_signal` ne tire que pour `location_access_pattern = 'destination_catchment'` (owner) | date, location_id, action_type, action_priority, action_category, channel_hint, headline_fr, detail_fr, data_payload, suppression_key, expires_at |
...
| `fct_location_sales_regime` 🆕 | table | location_id | fct_client_daily_performance | location_id, window_start, window_end, active_days, span_days, med_days_per_week, active_weeks, span_weeks, week_coverage, med_daily_txn, revenue_p25, revenue_med, revenue_p75, iqr_ratio, sales_grain |
...
| `fct_region_foreign_country_profile` 🆕 | table, clustered | date × region_code × country_name_fr | int_region_foreign_tourism_mix, dim_calendar | date, region_code, region_name, season, reference_year, accommodation_type, country_name_fr, nights_thousands, yoy_pct_change, pct_nonresident, country_share_of_nonresident, source_reliability **DEUX CODAGES RÉGIONAUX INCOMPATIBLES — vérifié en base 26/08.** Ce mart porte `region_code` au codage INSEE-mart (`FR81`, `FRC1`, `FRB0`, `FRD1`, `FRE1`) tandis que les LIEUX portent le NUTS-2016 (`FRJ`, `FRC`, `FRB`, `FRD`, `FRE`). Seule l'Île-de-France coïncide (`FR10`). **Aucune règle de préfixe ne marche** : quatre régions suivent `FRx → FRx1/FRx0`, l'Occitanie casse le motif (`FRJ → FR81`). La correspondance doit être EXPLICITE — elle vit en dur dans `fct_location_daily_action_candidates.sql:586-591` (`unnest([struct(...)])`), jamais par ressemblance de libellé. **Deux autres pièges du même mart** : (a) les parts par `accommodation_type` NE S'ADDITIONNENT PAS d'un type d'hébergement à l'autre — un CTE doit CHOISIR son type, jamais sommer (Île-de-France ne publie que `hotels`) ; (b) le profil INSEE ne couvre que les **mois 4 à 9**, et les lieux de PACA sont absents du mart. |
...
| `dim_client_company_profile` | view | location_id | client_company_profile, int_client_website_profiles | location_id, company_name, company_activity_type, position, location_type, event_time_profile, primary_audience_1/2, origin_city_id_1/2/3, origin_city_labels, commerce_type, commerce_gamme |
...
| `stg_day_notes` 🆕 | view | src:analytics.day_notes — **EN BASE** 07/09 (PR [ms_database#126](https://github.com/Ajuria/ms_database/pull/126) mergée `cbde545`, build run 70471896420048 vert) |
| `stg_explorer_site_memory` 🆕 | view | src:raw.explorer_site_memory (déclarée dans `staging/app_activity/sources.yml`, source `raw`) — **PR OUVERTE** 11/09 ([ms_database#143](https://github.com/Ajuria/ms_database/pull/143)). Table raw : app-write depuis `POST /api/explorer/agent` (insertion en flux, append-only), 9 colonnes REQUIRED, EU, 0 ligne au 11/09 08:45. À côté : `raw.explorer_agent_turns` (8 colonnes, trace des tours, app-write, AUCUN modèle dbt — pas de lecteur). |
...
| `stg_client_transactions` 🆕 | view | src:raw.client_transactions, src:analytics.import_routing_rules, src:analytics.import_invoice_parties, src:analytics.party_directory **10/09 (ms_database#134, run 70471896860859) : `units_sold` INT64 — une pesée compte pour une vente (owner) : quantité non entière → signe de la pesée (1, retour −1), sinon `quantity`, NULL conservé. Lu à la place de `quantity` par int_client_offering_profile, fct_client_hourly_sales, fct_client_item_signals_daily, fct_client_offering_daily. En base : 202 895 lignes, units_sold = quantity partout (0 pesée importée).** |
...
| `int_client_offering_profile` 🆕 | table | stg_client_transactions **10/09 (#134) : `units_30d` = Σ coalesce(units_sold, 1).** |
...
| `fct_client_hourly_signals_daily` 🆕 | table | location_id × transaction_date × transaction_hour — part de l'heure dans le CA du jour vs sa moyenne 30 j (z ≥ 2, ≥ 8 pts, n ≥ 20, calibré 23/08 : 16 % des jours-site) ; source `fct_client_hourly_sales` ; consommé par le CTE `hour_share_move` ; **journée arrêtée (06/09, HANDOFF-journee-arretee)** : ≥ 2 heures de queue à zéro après la dernière vente ⇒ `is_day_stopped`, `is_stop_row` (première heure à zéro), `last_sale_hour`, `typical_last_hour`, `stopped_hours`, `stop_hour`, `stopped_expected_revenue`/`_transactions` (attendus de la queue), `expected_revenue_until_stop`, `n_stopped_days_90d`, `stopped_dates_90d` — la queue n'est plus `is_hour_move` ; **familles de l'heure (06/09, HANDOFF-familles-heure)** : `hour_family_gaps` JSON [{family, revenue, expected, delta}] × 3, part typique poolée 8 mêmes dow × attendu de l'heure, source `stg_client_transactions`, NULL hors mouvement |
...
| `raw.competitor_directory.commerce_type / commerce_gamme` et `raw.insight_event_user_location_profile.commerce_type / commerce_gamme` (10/09, colonnes additives : type de commerce `specialiste`/`generaliste` et gamme `entree`/`milieu`/`haut`/`luxe`, déclarés, NULL = inconnu ; portées par stg → int → `fct_competitor_directory` et stg → `int_client_website_profiles` → `dim_client_company_profile`, lues par `int_competitor_threat_profile` — règle dans `with_tiers` : dans le commerce, type différent ou gamme à deux niveaux ou plus ⇒ au plus `partial` — → `fct_competitor_threat_profile` ; EN BASE 10/09, ms_database#140, run 70471896907689 ; `docs/type-et-gamme-de-commerce-spec.md`) |  |  |  |
...
| `mart.fct_client_family_price_daily` (cartes `family_price_move` et `family_discount_move`, blocs candidats 06/09 : un tir par site et par jour, famille au \|z\| le plus fort ; is_discount_move exige units ≥ 10 depuis le 06/09) | site × jour × famille | stg_client_transactions | prix réalisé (CA/unités), taux de remise ; base 28 j ; is_price_move (units ≥ 10), is_discount_move. **`has_fractional_units` (BOOL) : EN BASE 10/09 (PR ms_database#132, run 70471896850393 ; `docs/dbt-handoff/HANDOFF-prix-moyen-unite-2026-09-10.md`)** : une quantité non entière sur la famille, ce jour ou dans les 28 jours précédents ; `units` est alors une somme de poids et `realized_price` un prix au poids. Déclarée au contrat de `vw_insight_event_family_price_daily` (`select *` sous contrat enforced : sans elle, son build échoue). Mesuré 10/09 : 5 048 lignes, 0 `true` |
| `mart.fct_client_item_absence_daily` | site × jour × produit (absences) | stg_client_transactions | carte `item_absent_regular` (bloc candidat 06/09 : une carte par site et par jour, absents classés par CA habituel) ; produit régulier (≥ 80 % des 60 jours d'ouverture) absent un jour d'ouverture : is_absent_regular, expected_item_revenue |
| `mart.fct_client_day_decomposition` | site × jour | fct_client_day_residual × fct_client_daily_performance × fct_client_offering_daily | volume_term_eur + basket_term_eur = CA − attendu (exact) ; mix : top_families (spine 30 j, parts en ratio de sommes, Σ = gap), families_unexplained_eur, dominant_factor (porte de signe) **08/09 (ms_database#129, marts en base — run 70471896583230) : `fct_client_day_family_decomposition` + `baseline_unit_price`, `expected_units` (CA attendu de la famille ÷ prix unitaire de la fenêtre, MÊME référent que `expected_revenue`), `delta_units` ; `top_families` porte `expected_units` et `delta_units`. `baseline_units_per_day` (moyenne brute 30 j) n'est pas un attendu.** |
| `semantic.vw_insight_event_day_decomposition · vw_insight_event_tickets_daily · vw_insight_event_family_price_daily · vw_insight_event_item_absence_daily` | id. | les 4 marts ci-dessus | projections fidèles, contrat enforced (schéma généré du réel) — surfaces de lecture de l'app **08/09 (#130) : contrats enforced au niveau des marts — `units` INT64 sur la vue famille, struct `top_families` complète (units, baseline_units_per_day, expected_units, delta_units) ; le job quotidien était rouge dessus depuis le 08/09 05:00.** |
