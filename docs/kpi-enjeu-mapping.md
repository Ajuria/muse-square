...
> reste métadonnée d'apprentissage). Incrément 1 (store) LIVRÉ le 24/07 soir. ÉTAPE 2 LIVRÉE 24/07 nuit (+2.5 « facteurs mêlés »). ÉTAPE 3 LIVRÉE 26/07 : boucle multi-KPI (lib/kpiRegistry.ts — measured_metric réel = f(type,driver), kpi_baseline/window/delta additifs, verdict K1 inchangé ; 'reputation' clé posée mesure NULL faute de série own-rating). ÉTAPE 4 LIVRÉE 26/07 : pill K6 (discount_no_lift, classe coût), pill K3 via classe traffic_high (résiduel des jours à forte affluence — le contrefactuel « manque à convertir » écarté comme fabrication), chip « Suivi : note Google » sur le trio réputation, DÉMOTIONS actives (DEMOTED_TO_FEED). Reste : verdict par KPI quand variances établies ; surface days.ts/insight pour les pills ; cartes structurelles.
