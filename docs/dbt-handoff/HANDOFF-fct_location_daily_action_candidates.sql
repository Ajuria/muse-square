/*
  ...
  PURPOSE
    ...
    (score_driver_shift a été SUPPRIMÉ — démué au fil, cf. commentaire dans le corps) :
      ...
      sales_underperformance            — CA < 70% de la moyenne 30j, cause attribuée
...
change_feed_actions as (
    ...
    where cf.feed_date >= current_date()
      ...
      and (cf.change_subtype != 'mobility_disruption' or cf.mobility_disruption_flag_region = true)
...
-- ============================================================
-- ACTION TYPE S1: UNDERPERFORMANCE ATTRIBUTED
-- Revenue < 70% of 30d avg. Muse Square names the cause.
-- ============================================================

sales_underperformance as (
    select
        cp.transaction_date as date,
        cp.location_id,
        'sales_underperformance' as action_type,
        case
            when ds.opportunity_score_final_local >= 70 then 4
            else 3
        end as action_priority,
        'performance' as action_category,
        'note_interne' as channel_hint,
        concat(
            'CA en retrait de ',
            cast(round(abs(cp.revenue_vs_30d_avg_pct) * 100, 0) as string),
            '% - cause : ',
            coalesce(ds.primary_score_driver_label, 'contexte general')
        ) as headline_fr,
        concat(
            'CA du ',
            case extract(dayofweek from cp.transaction_date)
            ...
            ' ', format_date('%d/%m', cp.transaction_date), ' : ',
            cast(round(cp.daily_revenue, 0) as string), ' EUR vs ',
            cast(round(cp.revenue_30d_avg, 0) as string), ' EUR en moyenne 30j (-',
            cast(round(abs(cp.revenue_vs_30d_avg_pct) * 100, 0) as string), '%). ',
            case
                when safe_divide(ds.competition_index_local, ds.baseline_comp_avg) > 1.3
                    then concat('Cause probable : pression concurrentielle x',
                         cast(round(safe_divide(ds.competition_index_local, ds.baseline_comp_avg), 1) as string),
                         ' (', cast(ds.events_within_5km_count as string), ' evenements a 5 km).',
                         case when tt.competitor_name is not null
                              then concat(' Principal : ', tt.competitor_name, '.')
                              else '' end)
                when ds.alert_level_max >= 2
                    then 'Cause probable : conditions meteo defavorables.'
                when ds.primary_score_driver_label is not null
                    then concat('Facteur dominant : ', ds.primary_score_driver_label, '.')
                else 'Pas de facteur externe dominant identifie.'
            end
        ) as detail_fr,
        to_json_string(struct(
            cp.daily_revenue,
            round(cp.revenue_30d_avg, 0) as avg_30d,
            round(cp.revenue_vs_30d_avg_pct * 100, 1) as revenue_vs_avg_pct,
            round(safe_divide(ds.competition_index_local, ds.baseline_comp_avg), 2) as pressure_ratio,
        ...
        concat('sales_underperformance:', cp.location_id, ':', cast(cp.transaction_date as string)) as suppression_key,
        cp.transaction_date as expires_at
    from client_perf cp
    inner join daily_state ds
        on cp.location_id = ds.location_id
    ...
    left join top_threat tt
        on cp.location_id = tt.location_id
    where cp.daily_revenue < cp.revenue_30d_avg * 0.7
      and cp.revenue_30d_avg is not null
      and cp.revenue_30d_avg > 0
),
...
competitor_repricing_event as (
    select
        ...
        concat('competitor_repricing_event:', agg.location_id, ':', agg.competitor_id, ':', cast(current_date() as string)) as suppression_key,
        current_date() as expires_at
    from (
        select
            f.location_id,
...
-- C1: PERFECT STORM — 3+ favorable factors aligned
perfect_storm as (
    select
        d.date,
        d.location_id,
        'perfect_storm' as action_type,
        4 as action_priority,
        'opportunity' as action_category,
        'gbp' as channel_hint,
        'Conditions exceptionnelles — tous les feux sont au vert' as headline_fr,
        'Plusieurs facteurs favorables alignes simultanement.' as detail_fr,
        to_json_string(struct(
            d.opportunity_score_final_local as score,
            d.opportunity_regime as regime,
            d.alert_level_max as weather_alert,
            ...
            (case when d.alert_level_max = 0 then 1 else 0 end
             + case when d.pressure_ratio < 0.8 then 1 else 0 end
             + case when d.is_public_holiday_flag then 1 else 0 end
             + case when d.is_school_holiday_flag then 1 else 0 end
             + case when d.is_weekend_flag then 1 else 0 end
            ) as favorable_count
        )) as data_payload,
        concat('perfect_storm:', d.location_id, ':', cast(d.date as string)) as suppression_key,
        d.date as expires_at
    from daily_state d
    where d.date >= current_date()
      and d.date <= date_add(current_date(), interval 3 day)
      ...
      and d.opportunity_score_final_local >= 60
),
...
-- C3: SATURATED + BAD WEATHER
saturated_bad_weather as (
    select
        d.date, d.location_id,
        'saturated_bad_weather' as action_type,
        4 as action_priority,
        'competition' as action_category,
        'internal' as channel_hint,
        'Mauvais temps + saturation concurrentielle' as headline_fr,
        'Double risque : meteo defavorable et saturation sectorielle.' as detail_fr,
        to_json_string(struct(
            d.alert_level_max as weather_alert,
            round(d.pct_same_bucket_5km * 100, 1) as pct_same_sector,
            d.events_within_5km_count as events_5km,
        ...
        concat('saturated_bad_wx:', d.location_id, ':', cast(d.date as string)) as suppression_key,
        d.date as expires_at
    from daily_state d
    where d.date >= current_date()
      and d.date <= date_add(current_date(), interval 3 day)
...
-- C11: EXTENDED BAD WEATHER 3D (extends existing 2d to 3d window)
extended_bad_weather_3d as (
    select
        w.date, w.location_id,
        'extended_bad_weather_3d' as action_type,
        4 as action_priority,
        'weather' as action_category,
        'internal' as channel_hint,
        'Meteo defavorable 3+ jours consecutifs' as headline_fr,
        'Alerte meteo prolongee sur 3 jours ou plus.' as detail_fr,
        to_json_string(struct(
            w.alert_level_max as alert_level,
            l.weather_sensitivity as site_sensitivity
        )) as data_payload,
        concat('ext_bad_wx_3d:', w.location_id, ':', cast(w.date as string)) as suppression_key,
        w.date as expires_at
    from weather w
    inner join loc l on w.location_id = l.location_id
    where w.date = current_date()
      and w.alert_level_max >= 2
      and exists (
          select 1 from weather w2
          where w2.location_id = w.location_id
            and w2.date = date_add(w.date, interval 1 day)
            and w2.alert_level_max >= 2
      )
      and exists (
          select 1 from weather w3
          where w3.location_id = w.location_id
            and w3.date = date_add(w.date, interval 2 day)
            and w3.alert_level_max >= 2
      )
),

-- C19: WEATHER + MOBILITY DOUBLE HIT
weather_mobility_double as (
    select
        d.date, d.location_id,
        'weather_mobility_double' as action_type,
        4 as action_priority,
        'weather' as action_category,
        'internal' as channel_hint,
        'Double alerte : meteo + mobilite' as headline_fr,
        'Meteo defavorable et perturbation de mobilite simultanees.' as detail_fr,
        to_json_string(struct(
            d.alert_level_max as weather_alert,
            (d.delta_att_mobility_pct < 0) as mobility_disrupted,
            round(d.delta_att_mobility_pct, 1) as mobility_delta_pct
        )) as data_payload,
        concat('wx_mob_double:', d.location_id, ':', cast(d.date as string)) as suppression_key,
        d.date as expires_at
    from daily_state d
    where d.date >= current_date()
      and d.date <= date_add(current_date(), interval 3 day)
...
all_candidates as (
    ...
    union all
    select * from sales_underperformance
    union all
    ...
    union all
    select * from perfect_storm
    union all
    select * from weather_comp_opportunity
    union all
    select * from saturated_bad_weather
    union all
    ...
    union all
    select * from extended_bad_weather_3d
    union all
    select * from weather_mobility_double
    union all
