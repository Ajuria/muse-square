...
| Jour inexpliqué sans note | `semantic.vw_insight_event_day_residual` : \|`residual_z`\| ≥ 2, ≤ 30 j, ET aucune ligne dans `analytics.day_notes` (même site, même jour) | La carte porte SA saisie : `POST /api/insight/day-notes` (E3, **livré 07/09**) — forme owner « Un souvenir ? Notez-le · sinon, laissez », bouton « Enregistrer ». **La même carte est rendue en tête du fil Agir depuis le 08/09 (owner « Go with B ») — avant « Vos actions du jour », mêmes chaînes, même rail ; un MEMBRE lit la forme en ventes (owner 08/09 : « ventes ») — « Mardi 01/09 : 350 ventes, +77 % vs vos ventes habituelles », jour inexpliqué dans la couche volume (\|z ventes\| ≥ 2).** Un jour = une carte : quand la question mesurée (nature 3) porte la même date, elle cède. |
