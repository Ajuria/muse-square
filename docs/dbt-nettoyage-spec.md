...
Hors de cette simulation, volontairement : la chaîne `refresh_address` (3 modèles, décision de câblage au geste 6), `stg_competitor_tracking` (entre dans `attractivite-spec.md`), `int_calendar_event_spans` (future source du directory), les 6 modèles lus par l'owner, et les 25 modèles tourisme (exempts).
...
| 7 | Les trois marts de grain (audit § 5) et [`tourisme-grain-spec.md`](tourisme-grain-spec.md), [`attractivite-spec.md`](attractivite-spec.md) | 3, 6 | chacun vérifié sur `f10c3e58` |
