...
| Demander à l'exploitant le bilan d'un engagement résolu (carte « bilan » de l'état vide d'Explorer, bloc « Documenter » de la page de l'engagement — retour owner 12-13/09 : « Votre bilan ajoute ce que la mesure ne voit pas — 2 minutes. » ne disait pas le gain ; « relu / repris » est un mécanisme, pas un gain ; les formes-slogans « corriger le tir », « clé du succès », « levier gagnant » refusées comme images) | **RATIFIÉ 13/09 — la ligne dit le GAIN avec le chiffre de l'action** : **−394 € : ce que vous changez au prochain « Corner de vente producteur ».** · **+612 € : ce qui a marché, à refaire au prochain « Corner de vente producteur ».** · **La raison du report — la journée ne compte pas contre le dispositif.** (non menée) · **Ce que vous avez vu ce jour-là, pour trancher au prochain « … ».** (non concluant) ; « la prochaine fois » quand le titre dépasse 40 caractères. Le bilan tient en UNE ligne + **À reproduire ?** ; l'intitulé du champ suit l'état : **Ce que vous changez** · **Ce qui a marché** · **La raison du report** · **Le constat terrain pour trancher** (mots owner 13/09), invite **En une ligne**. Un seul foyer : `commitmentCopy.retro_gain_*` (`retroGainFr`). Le titre de la carte, quand l'objectif porte sur une famille, dit le référentiel : **… manqué · votre lieu +39 € sur 1 jour** | ajoute ce que la mesure ne voit pas, 2 minutes, relu / repris, corriger le tir, clé du succès, levier gagnant, inverser la tendance, neutraliser, feedback |
| Demander à l'exploitant le bilan d'une OPÉRATION (page de l'opération, étape « Évaluer » ; le courriel J+7 ; la tuile « Faire le bilan » du tableau garde son bénéfice owner du 17/08) | **RATIFIÉ 13/09** : « Le résultat est mesuré ci-dessous. Pour que ce jour soit jugé à conditions égales — météo, accès, fréquentation — trois / quatre réponses, une minute. » ; l'intro du bloc : « Pour juger ce jour à conditions égales : la météo, l’accès et la fréquentation que vous avez vus, et le dispositif appliqué ou non. Le CA, lui, est déjà mesuré ci-dessus. » ; courriel : **Votre bilan — <titre>** | post-événement (calque), votre bilan ajoute ce que la mesure ne voit pas |
| Demander à l'exploitant une photo de composant (bloc « Composants » de la page du pôle) | **Documenter →** (CTA owner 03/09) ; sous le titre du bloc, une fois : **Prenez une photo de votre dispositif : vous verrez ce que chaque agencement donne sur vos ventes.** (phrase OWNER du 13/09, franglais « set-up » retiré — règle 7 bis ; forme « le gain dit, pas promis », choisie par lui). Affichée dès la PREMIÈRE version : sans photo à la V1, il n'y a rien à comparer à la V2. Quatre propositions de Claude refusées avant — trois énuméraient ce que la photo capte (règle 8), la quatrième promettait « quand l'objectif a été atteint » alors qu'un dispositif permanent n'a NI fenêtre NI verdict (owner 27/08) | prenez une photo (nu), inventoriez, scannez votre rayon, set-up (franglais), toute ligne qui énumère ce qui est capté, tout mot de verdict sur un pôle |
| Créer la version suivante d'un PÔLE (13/09, page du pôle, owner seul) | un volet replié **La version suivante** (titre déjà en prod sur les opérations), sous-titre « modifiez le pôle, ses composants et leurs mesures, puis enregistrez », qui contient LE formulaire de pôle pré-rempli avec la version courante ; son bouton dit **Enregistrer →**. **Aucune question** (owner 13/09 : « le versionning du pôle est déclaratif — sa page de réglages ; la question vit au composant, avec la photo ») et **aucun nouveau bouton** : « Ajuster → » existe déjà sur le Tableau de bord et mène à cette page | Qu'avez-vous changé ? (au pôle — reste au composant), Ajuster (dupliqué), Nouvelle version, V2 (abréviation), tout mot de verdict sur un pôle |
| L'historique du dispositif montre la photo de chaque version à côté de son verdict (13/09) | vignette + **libellé du type du composant** (Vitrine, Linéaire…) + date ; une version sans photo dit **Aucune photo de cette version.** (même forme que « Aucun composant déclaré pour l'instant. ») ; au-delà de quatre vignettes, **+ N autres** | mémoire visuelle (mot de chantier, jamais à l'écran), archive, galerie |
| Ce que l'agent dit quand il vient d'enregistrer ce que l'exploitant lui a dit de son espace | **C'est noté : vous n'aurez pas à me le redire.** (règle du prompt système, 13/09 — dire ce que ça permet, jamais la mécanique ; ne promet aucun changement de chiffre : la mémoire ne change aucune mesure) | j'ai bien enregistré, c'est sauvegardé, dans ma mémoire |
| Demander à l'exploitant la cause d'un jour hors du commun (carte « jour inexpliqué » — état vide d'Explorer et Pulse) | **Si vous savez pourquoi, notez-le : la cause revient avec ce jour.** (proposé 13/09, à ratifier — remplace « Un souvenir ? Notez-le · sinon, laissez », owner 22/08, qui reconnaissait qu'on ne se souvient pas mais ne disait aucun gain : zéro note écrite sur le compte de test). La permission reste (« si vous savez »), le gain est VÉRIFIÉ : la note est citée par le chat quand ce jour revient (`buildDayPerformanceFacts`). Ce qu'elle ne fait pas, donc ce qui ne s'écrit pas : elle n'exclut pas le jour du résultat habituel. Bouton **Enregistrer** | un souvenir ?, votre mémoire, ne perdez pas, tracez la cause |
...
| Les trois verdicts de cadrage du croquis, sur l'écran d'accueil du Relevé | **entier, de face** · **trop près** · **de biais** (owner 14/09, piste A) — deux mots chacun, sous un cadre qui les MONTRE | bon / mauvais (un jugement, pas une observation), cadrage correct, recadrez |
| Ce qui s'affiche quand une note est bien écrite dans le Rapport mais n'a pas rejoint la mémoire du site | **Note enregistrée dans ce rapport uniquement.** (owner 14/09) — « uniquement » porte tout le sens ; en ambre, comme les notes, jamais en rouge : le geste a RÉUSSI | expliquer le mécanisme (« la mémoire de votre commerce », « l'agent ne la relira pas ») — l'exploitant n'a pas à le connaître pour comprendre ce qui manque ; Échec, Erreur |
| Le geste qui ouvre l'éditeur d'un bloc de Rapport, et qui CONTIENT les deux modes | **Modifier** (owner 14/09) — s'accorde avec **Modifier la Synthèse**, le même geste au niveau du document | Écrire comme nom du BOUTON (c'est l'un des deux modes : le nommer ainsi laisserait croire que l'autre est ailleurs), Éditer (règle 7 bis) |
| Les deux positions de l'interrupteur qui ouvre l'éditeur d'un bloc de Rapport — ce que l'exploitant s'apprête à FAIRE, avant d'avoir écrit | **Écrire** · **Demander** (owner 14/09) | Votre note / Approfondir comme LIBELLÉS DE BOUTON (ils nomment ce qui est PRODUIT, deux lignes plus haut, et le restent) ; Noter, Poser une question, Ajouter |
...
| Le NOM d'un composant dans l'app | **celui que porte la donnée** — `component_label`, par ex. « N° 2 — Vin & Spiritueux », « N° 24 — Ilot maison » (52 sur 52 chez Muse Square). Il porte le numéro ET le repère écrit sur le plan : c'est cette seconde moitié qui le rend reconnaissable. | **tout nom recomposé** à partir d'autres champs (« N° 7 · Cave » = numéro + pôle, owner 15/09 : « je ne comprends pas ») ; une clé interne à l'écran (« p7 ») |
| Déplacer une photo posée sur le mauvais composant | **Déplacer →** (le bouton, à côté de « Retirer → ») · **ON NE TAPE PLUS UN CODE** : la liste montre les composants avec leur **dernière photo** et leur **longueur de façade** — « Sur quel composant ? », « Filtrer par nom ou par numéro », « pas encore de photo », « autre pôle », « Aucun composant ne correspond. » (owner 15/09 : « la liste avec photos d'abord » ; mesuré le même jour : le plan ne porte AUCUN numéro, et 40 des 52 libellés sont ambigus). Le numéro reste un raccourci dans le filtre · la ligne quittée dit **« Aucune photo ici — vous l'avez déplacée sur le {nom du composant}. »** avec **Annuler** · la ligne d'arrivée ne dit **rien de plus** qu'une photo ordinaire. Le statut stocké est **`"déplacée"`**, accentué (owner 15/09 : « écris en bon français »). Owner 15/09, mots ratifiés. | « cette photo est passée au… », « arrivée du n°… » (le mécanisme, et le trajet raconté à qui vient de le faire) ; `"deplacee"` sans accent |
| Déposer le plan du magasin (bandeau de la vue Dispositif) | Titre **Le plan de votre magasin** · écran d'envoi **Déposez le plan de votre magasin** + **Vous y retrouverez vos numéros quand vous corrigez une photo.** · bouton **Déposer →** (il ne répète pas le titre : constaté au rendu) · **Remplacer** · **Déposé le {date}** · **Ouvrir le plan →**. **Déposez le plan de votre magasin** — celui de l'exploitant, avec ses numéros déjà imprimés (owner 15/09, voie b). | relever les meubles un par un sur le plan ; « importer », « téléverser » (règle 7 bis) |
| Placer les composants sur le plan, en fin de pôle | **Placer sur le plan** (le bouton) · **Placer ce pôle sur votre plan** · **{nom} · {m} m de façade** · **Touchez son emplacement sur le plan.** · **{fait} sur {total} placés** · **Ce pôle est placé.** · et, si le plan est un PDF, **Pour placer vos composants, déposez votre plan en image (PNG ou JPEG)…** (owner 15/09). La longueur de façade accompagne le nom : c'est elle qui départage les homonymes. | 52 clics au bureau ; annoncer un composant sans sa longueur (40 sur 52 portent un libellé partagé) |
| Le verdict d'une photo, pendant la marche | **Gardée** · **un peu floue** / **un peu sombre** · **Reprendre** (owner 15/09). Il s'affiche sous le viseur avec la VIGNETTE, quatre secondes, et disparaît seul. Il ne dit que ce que la marche MESURE — netteté et lumière ; le cadrage est lu par le serveur et ne s'annonce pas ici. | annoncer le cadrage avant l'envoi ; « ratée », « mauvaise » (l'image est gardée, avec son défaut nommé) |
| Le bilan d'une marche, sur sa page de fin | **Ce que cette marche a donné** · **{n} photos gardées en {min} min** · **{n} écartées : {motifs}** avec **trop sombre** / **trop floue** · **Aucune écartée.** · et, quand le contexte de la marche n'est pas parti, **Le détail de cette marche n'a pas été enregistré. Vos photos, elles, sont en base.** (owner 15/09). Les écarts se disent par leur MOTIF : c'est lui qui se corrige à la marche suivante. | un compte sans motif ; « perdues », « ratées » (l'appareil REFUSE avant de garder, il ne rate rien) |
| Les deux vues de la page d'un pôle | **Dispositif** (comment le pôle est monté : son espace, ce qu'il rapporte, ce que les photos ont appris, ses composants, son historique, sa version suivante) · **Performance** (ses résultats et leur vue jour, son classement, la décomposition, ses familles, ses opérations). Arbitrées sur proto, câblées dans la page le 15/09. Le **plan** reste en BANDEAU au-dessus des onglets : il dit « où suis-je », ce qui vaut pour les deux. | Réglages, Set-up (règle 7 bis), Vue d'ensemble, Données |
| Les deux sections de l'espace sur la page d'un pôle — comment il est monté, et ce qu'il produit à cette place | **Comment ce pôle est installé** (mètres de linéaire, Part de linéaire, surface de vente — aucune fenêtre : ce ne sont pas des quantités de 30 jours) · **Ce que ce pôle rapporte — 30 derniers jours** (€ par mètre linéaire, par m², marge brute, part du CA contre part de linéaire). Owner 15/09 : « data shall answer 2 questions : how is the Pole set up + how does it compare to other poles », puis correction du même jour — « Ce que cette **place** rapporte → **Non** → Ce que ce **pôle** rapporte ». | **place** comme synonyme de pôle (le mot du 27/08 existe : un concept = LE mot) ; « Espace — 30 derniers jours » en titre unique, qui datait les mètres |
