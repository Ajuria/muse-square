...
| Créer la version suivante d'un PÔLE (13/09, page du pôle, owner seul) | section **La version suivante** (titre déjà en prod sur les opérations), question **Qu'avez-vous changé ?** (idem), bouton **Enregistrer →** (le verbe ratifié le même jour pour le Rapport : le clic ÉCRIT) — la réponse devient « Le plus du dispositif » de la version nouvelle ; tout le reste est hérité | Nouvelle version, Dupliquer le pôle, V2 (abréviation), tout mot de verdict sur un pôle |
