...
| La raison pour laquelle on va dans un commerce — faire ses courses, ou un produit précis — déclarée pour le site ET pour chaque concurrent, lue par le calcul de menace (`concurrent-format-spec.md`) ; champ affiché SEULEMENT si le secteur est Commerce & Retail ou Marchés & Halles, vidé si le secteur change | champ **Type de commerce** · valeurs **Spécialiste** · **Généraliste** (owner 10/09) | Format (dit la taille : une supérette et une épicerie ont le même), Positionnement (marketing), Assortiment (centrale d'achat) ; enseigne, grande surface, discount, haut de gamme ; toute valeur déduite par l'app ou tirée d'un seed |
...
- **La définition de « Spécialiste » / « Généraliste »** : proposée, non validée — « On y va pour
  faire ses courses, ou pour un produit précis ? » (`concurrent-format-spec.md` § 2). Le champ
  « Type de commerce » et ses deux valeurs, eux, sont tranchés (owner 10/09).
