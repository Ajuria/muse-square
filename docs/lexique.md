...
| Demander à l'exploitant une photo de composant (bloc « Composants » de la page du pôle) | **Documenter →** (CTA owner 03/09, inchangé), SEUL — aucune ligne d'accroche sous le titre : deux propositions refusées le 13/09 (« Une photo par composant, et vous voyez ce qui est exposé sans se vendre… », « La photo garde votre agencement : ce qui est groupé avec quoi… »), toutes deux énumérant ce que l'app capte. La demande se réécrira quand l'historique du pôle montrera la photo de chaque version à côté de son verdict (règle 8, la surface d'abord) | prenez une photo (nu), inventoriez, scannez votre rayon, toute ligne qui énumère ce qui est capté |
