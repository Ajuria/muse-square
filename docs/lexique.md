...
| Déplacer une photo posée sur le mauvais composant | **Déplacer →** (le bouton, à côté de « Retirer → ») · la ligne quittée dit **« Aucune photo ici — vous l'avez déplacée sur le {nom du composant}. »** avec **Annuler** · la ligne d'arrivée ne dit **rien de plus** qu'une photo ordinaire. Le statut stocké est **`"déplacée"`**, accentué (owner 15/09 : « écris en bon français »). Owner 15/09, mots ratifiés. | « cette photo est passée au… », « arrivée du n°… » (le mécanisme, et le trajet raconté à qui vient de le faire) ; `"deplacee"` sans accent |
