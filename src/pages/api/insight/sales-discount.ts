...
// Card-SPECIFIC drill-down for sales_discount_no_lift ("Remises sans effet"). Answers the one question
// the operator can act on: do the days you discount actually earn MORE? Compares CA-vs-30d-avg on the
// venue's high-discount days vs its low-discount days over ~90 days. When the discount days do NOT
// outperform, the promos are wasted margin. Strictly ASSOCIATIVE (you may discount BECAUSE a day is
// slow) — framed as "vos remises n'achètent pas de CA incrémental", never "la remise fait baisser le CA".
// Source: mart.fct_client_sales_signals_daily — verified cols: discount_rate, discount_rate_baseline,
...
export const GET: APIRoute = async ({ url, locals }) => {
  try {
    ...
    if (all.length < MIN_DAYS) return json(200, { ok: true, found: false, date });
    ...
    return json(200, {
      ok: true, found: true, date, lead, no_lift: noLift,
      ...
      compare, scale, decision_lines,
      caveat: "Association observée sur la période, pas une preuve : un jour peut être remisé PARCE QU'il est calme. À confirmer par un jour-test sans remise.",
    });
  } catch (err: any) {
