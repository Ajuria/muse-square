...
// long they stay. THIN wrapper over the shared provider `audienceFamily`
// (src/lib/insightFamilies/audience.ts) — the SAME provider the family report and the grounded prompt
// Q&A reuse, so the declared-vs-BestTime distinction cannot drift between the deep page and the chat.
// Response is byte-identical to the pre-extraction endpoint: { ok, found, date, audience }.
import type { APIRoute } from "astro";
import { makeBQClient } from "../../../lib/bq";
import { requireLocationOwnership } from "../../../lib/requireLocationOwnership";
import { audienceFamily } from "../../../lib/insightFamilies/audience";
...
export const GET: APIRoute = async ({ url, locals }) => {
  try {
    ...
    const result = await audienceFamily(bq, location_id, date);
    return json(200, { ok: true, ...result.data });   // data carries found + date + audience
  } catch (err: any) {
