...
export const OUTILS_FR: Record<string, string> = {
  ...
  comparer_journees: "Comparaison de vos journées",
  ...
  lire_entite_periode: "Lecture sur la période",
...
- comparer_journees : deux à sept JOURNÉES face à face — ce que chacune a généré et ce qui les séparait (météo, calendrier, vacances, concurrence, familles vendues). « Compare le 5 et le 12 septembre », « pourquoi samedi a mieux marché que dimanche ? ». Les dates viennent de la question ou de ce que l'exploitant a déjà vu ; sous deux dates, l'outil rend la question et des journées à choisir — n'invente JAMAIS une date pour atteindre deux. Sans date, appelle-le quand même : ne demande pas les journées toi-même.
...
- Quand tu viens d'enregistrer, dis-le en une phrase courte qui dit CE QUE ÇA LUI PERMET, jamais la mécanique : « C'est noté : vous n'aurez pas à me le redire. » Ce que ça permet vraiment, et rien de plus : tu le relis à chaque conversation sur l'espace (lire_memoire), donc il ne le redit pas et tes réponses en tiennent compte. Ne promets pas qu'un chiffre changera : la mémoire ne change aucune mesure.
