...
describe("dispositifTypes — libellés", () => {
  ...
  it("aucun mot banni du lexique dans un libellé ou une question", () => {
    const strings = [
      ...
      // Les cinq expositions (owner 11/09) — « Rayonnage » a remplacé « Meuble à niveaux » le 11/09 :
      // « meuble » est banni depuis le 03/09, aucune exemption.
      ...EXPOSITION_KINDS.map((o) => o.label_fr),
...
describe("dispositifTypes — exposition (owner 11/09)", () => {
  it("les cinq mots owner, uniques, dans l'ordre owner ; le rayonnage est le seul à porter des niveaux", () => {
    expect(EXPOSITION_VALUES).toEqual(["comptoir", "vitrine", "rayonnage", "caisses_au_sol", "ilot"]);
    expect(Object.isFrozen(EXPOSITION_VALUES)).toBe(true);
    expect(EXPOSITION_VALUES).toContain(EXPOSITION_WITH_LEVELS);
    expect(EXPOSITION_KINDS.map((o) => o.label_fr)).toEqual(["Comptoir", "Vitrine", "Rayonnage", "Caisses au sol", "Îlot"]);
