...
const TARIFS_PATTERNS = [
  "tarif", "tarifs", "billetterie", "billet", "billets",
  "prix", "reservation", "réservation", "reserver", "réserver",
  "booking", "book", "tickets", "ticket", "shop", "boutique",
  "store", "achat", "acheter", "pricing", "rates", "buy",
  "abonnement", "abonnements", "pass", "visites", "visiter",
];

function scoreUrlBy(url: string, patterns: string[]): number {
  try {
    const path = new URL(url).pathname.toLowerCase();
    return patterns.filter((p) => path.includes(p)).length;
  } catch {
    return 0;
  }
}
...
function scoreUrl(url: string): number {
  return scoreUrlBy(url, AGENDA_PATTERNS);
}

function pathMatchesPatterns(url: string, patterns: string[]): boolean {
  try {
    const path = new URL(url).pathname.toLowerCase();
...
export async function discoverTarifsUrl(
  sourceUrl: string,
  browserlessToken: string,
  timeoutMs = 10_000
): Promise<DiscoveryResult> {
  try {
    ...
    const candidates = allLinks
      .filter((h) => {
        try {
          return new URL(h).origin === origin && pathMatchesPatterns(h, TARIFS_PATTERNS);
        } catch {
          return false;
        }
      })
      .filter((v, i, a) => a.indexOf(v) === i)
      .map((h) => ({ url: h, score: scoreUrlBy(h, TARIFS_PATTERNS) }))
      .sort((a, b) => b.score - a.score)
      .slice(0, 3);
    ...
    console.error("[url-discovery:tarifs]", err?.message);
    return { discovered_url: null, discovery_status: "error" };
  }
}