...
//   npx dotenv -e .env -- npx tsx scripts/explorer-quality-battery.ts [port]
...
(async () => {
  ...
  let baseline: any[] = [];
  try { baseline = JSON.parse(fs.readFileSync("explorer-battery-baseline.json", "utf8")); } catch { /* first run */ }
  ...
  fs.writeFileSync("explorer-battery-report.md", lines.join("\n"));
  fs.writeFileSync("explorer-battery-baseline.json", JSON.stringify(results.map(({ q, producer, seconds, verdict, avg, judge: j }) => ({ q, producer, seconds, verdict, avg, worst: j?.worst })), null, 1));
